...
//     printf("[DEBUG] handle_xread_block: fd=%d, num_keys=%d, timeout_ms=%d\n", fd, num_keys, timeout_ms);
//     for (int i = 0; i < num_keys; i++) {
//         printf("[DEBUG] handle_xread_block: checking key=%s, id=%s\n", keys[i], ids[i]);
...
//                 printf("[DEBUG] handle_xread_block: found new entries for key=%s, responding immediately\n", keys[i]);
//                 write(fd, buffer, strlen(buffer));
//                 found = 1;
//                 break;
//             }
...
//     printf("[DEBUG] handle_xread_block: blocking client fd=%d\n", fd);
...
//     printf("notified after xadd\n");
//     while (node) {
//         waiting_client_xread* wc = (waiting_client_xread*) node->value;
//         printf("[DEBUG] notify_xread_clients: checking fd=%d for key=%s\n", wc->fd, key);
//         int found = 0;
//         char buffer[1024 * 8] = {0};
...
//                 if (entry && entry->value_type == TYPE_STREAM) {
//                     stream* s = (stream*) entry->value;
//                     printf("[DEBUG] notify_xread_clients: checking stream for key=%s\n", key);
...
//                         printf("[DEBUG] notify_xread_clients: notifying fd=%d with new entry for key=%s\n", wc->fd, key);
...
//     long long now = current_time_ms();
//     dll* node = xread_waiting_clients->head;
//     printf("[DEBUG] check_xread_timeouts: now=%lld\n", now);
//     while (node) {
//         waiting_client_xread* wc = (waiting_client_xread*) node->value;
//         printf("[DEBUG] check_xread_timeouts: fd=%d, timeout_ms=%d, elapsed=%lld\n", wc->fd, wc->timeout_ms, now - wc->start_time_ms);
//         if (wc->timeout_ms != -1 && (now - wc->start_time_ms >= wc->timeout_ms)) {
//             printf("[DEBUG] check_xread_timeouts: timeout for fd=%d, sending *0\\r\\n\n", wc->fd);
//             char buffer[8] = "*0\r\n";
//             write(wc->fd, buffer, strlen(buffer));
//             dll* to_remove = node;
//             node = node->next;
